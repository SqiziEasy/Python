...
def wrong_answer_kbd(update, context, chat_id):
    global wrong_answer_kbd
    query = update.callback_query
    w_a_kbd = [
        [InlineKeyboardButton(" Показать ответ", callback_data=("show_answer" + str(chat_id)))],
        [InlineKeyboardButton(" Перейти к следующему заданию", callback_data=("go_to_next_question" + str(chat_id)))],
        [InlineKeyboardButton(" Перейти к выбору заданий", callback_data=("set_n_of_task" + str(chat_id)))]
    ]

    reply_markup = InlineKeyboardMarkup(w_a_kbd)

    context.bot.send_message(chat_id=chat_id, text="Выберите команду", reply_markup=reply_markup)

#    def cb_taker(call, chat_id):
 #       if call.data == ("show_answer" + str(chat_id)):
  #          context.bot.send_message(chat_id=chat_id, text=objects[users[int(chat_id)]["current_object"]]["questions"][int(users[int(chat_id)]["current_task_number"])][crnt_q]["answer"])
   #     elif call.data == ("go_to_next_question" + str(chat_id)):
    #        giving_random_task(update, context)
...
def checking_question(update, context):
    ...
    answ = str(update.message.text)

    print(objects[users[int(chat_id)]["current_object"]]["questions"][int(users[int(chat_id)]["current_task_number"])][crnt_q]["answer"])

    if answ == objects[users[int(chat_id)]["current_object"]]["questions"][int(users[int(chat_id)]["current_task_number"])][crnt_q]["answer"]:
        ...
        #query = update.callback_query
        wrong_answer_kbd(update, context, chat_id)
        #query.answer()
        if call.data == "show_answer":
            query.edit_message_text(chat_id=chat_id, text=objects[users[int(chat_id)]["current_object"]]["questions"][int(users[int(chat_id)]["current_task_number"])][crnt_q]["answer"])
        elif call.data == "go_to_next_question":
            giving_random_task(update, context)

    #query.answer()
    #elif callback_data == "set_n_of_task":
...
def button(update, context):
    ...
    if command[0] == 'set_obj':
        ...
        global giving_random_task

        def giving_random_task(update, context):
            global crnt_q
    ...
    elif command[0] == "show_answer":
        context.bot.send_message(chat_id=command[2], text=objects[users[int(chat_id)]["current_object"]]["questions"][int(users[int(chat_id)]["current_task_number"])][crnt_q]["answer"])
